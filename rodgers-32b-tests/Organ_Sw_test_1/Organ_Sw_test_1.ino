...
const int highBPin = 3;  // pushbutton 2 pin
const int highCLedPin =  13;    // LED pin
const int highBLedPin =  22;    // LED pin


void setup()
{
  // Set up the pushbutton pins to be an input:
  pinMode(highCPin, INPUT);
  pinMode(highBPin, INPUT);

  // Set up the LED pin to be an output:
  pinMode(highCLedPin, OUTPUT);      
  pinMode(highBLedPin, OUTPUT);
}


void loop()
{
  int highCState, highBState;  // variables to hold the pushbutton states

  highCState = digitalRead(highCPin);
  highBState = digitalRead(highBPin);

  if (highCState == LOW)   // if high c is connecting the circuit to ground (making it LOW)
         
  {
    digitalWrite(highCLedPin, HIGH);  // turn the LED on
  }
  else
  {
    digitalWrite(highCLedPin, LOW);  // turn the LED off
  }


  if (highBState == LOW)   // if high b is connecting the circuit to ground (making it LOW)
      
  {
    digitalWrite(highBLedPin, HIGH);  // turn the LED on
  }
  else
  {
    digitalWrite(highBLedPin, LOW);  // turn the LED off
  }
