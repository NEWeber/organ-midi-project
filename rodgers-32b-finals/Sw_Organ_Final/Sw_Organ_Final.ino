// Code for the swell keyboard. Each key wire is plugged into pins on an ATMEGA. 
...
void setup()
{
  ...
  for (int i = 0; i < numKeys; i++) 
  {
    pinMode(swNotes[i], INPUT_PULLUP);
  }
...
void loop()
{
  for (int noteCounter = 0; noteCounter < numKeys; noteCounter++) {
    ...
    if ((digitalRead(swNotes[noteCounter]) == LOW) && (keyPressed[noteCounter] == false))
    {
      keyPressed[noteCounter] = true;
      noteOn(noteCounter);
    }
    //if the key is released and it was held before, send note off and set keyPressed to false
    else if ((digitalRead(swNotes[noteCounter]) == HIGH) && (keyPressed[noteCounter] == true))
    {
